...
M.setup = function()
    VIM_LSP.handlers["textDocument/hover"] =
        VIM_LSP.with(
            VIM_LSP.handlers.hover,
            {
                border = "rounded",
            }
        )

    VIM_LSP.handlers["textDocument/signatureHelp"] = VIM_LSP.with(
        VIM_LSP.handlers.signature_help,
        {
            border = "rounded",
        }
    )
...
M.set_mappings = function(_, bufnr) -- (client, bufnr)
    ...
    VIM_KEYMAP_SET(
        { "n" },
        "<leader>gd",
        VIM_LSP.buf.definition,
        opts
    )
    VIM_KEYMAP_SET(
        { "n" },
        "<leader>gD",
        VIM_LSP.buf.declaration,
        opts
    )
    VIM_KEYMAP_SET(
        { "n" },
        "<leader>gt",
        VIM_LSP.buf.type_definition,
        opts
    )
    VIM_KEYMAP_SET(
        { "n" },
        "<leader>gi",
        VIM_LSP.buf.implementation,
        opts
    )
    VIM_KEYMAP_SET(
        { "n" },
...
M.set_autocmds = function(client, _) -- (client, bufnr)
    ...
    if client.server_capabilities.document_highlight then
        local group = VIM_API.nvim_create_augroup(
